...
std::vector<std::string> GestorComandos::dividir(const std::string& linha) {
...
void GestorComandos::processarComando(const std::string& linha) {
...
bool GestorComandos::validarComando(const std::vector<std::string>& palavras) {
    std::string cmd = palavras[0];
...
bool GestorComandos::validarJardim(const std::vector<std::string>& palavras) {
    if (palavras.size() != 3) {
...
bool GestorComandos::validarPlanta(const std::vector<std::string>& palavras) {
    if (palavras.size() != 3) {
        std::cout << "Erro: planta requer posição e tipo.\n";
        return false;
    }
    std::string pos = palavras[1];
    ...
    if (std::string("crex").find(tipo) == std::string::npos) {
...
bool GestorComandos::validarColhe(const std::vector<std::string>& palavras) {
    if (palavras.size() != 2) {
...
bool GestorComandos::validarAvanca(const std::vector<std::string>& palavras) {
    if (palavras.size() > 2) {
...
bool GestorComandos::validarCompra(const std::vector<std::string>& palavras) {
    if (palavras.size() != 2) {
        std::cout << "Erro: compra requer o tipo de ferramenta.\n";
        return false;
    }
    char tipo = palavras[1][0];
    if (std::string("gat z").find(tipo) == std::string::npos) {
